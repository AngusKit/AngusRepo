package cloud.xcan.angus.core.repo.infra.installation;
...
public class ConfigurableRepoApplication implements ConfigurableApplication {
  ...
  String REPO_SERVICE = "AngusRepo";

  /* AngusRepo */
  String REPO_HOST = "REPO_HOST";
  String REPO_PORT = "REPO_PORT";
  String REPO_WEBSITE = "REPO_WEBSITE";
  String REPO_PLUGIN_LOADED = "REPO_PLUGIN_LOADED";
  String REPO_PLUGIN_IGNORED = "REPO_PLUGIN_IGNORED";
  String REPO_APIS_SERVER_URL = "REPO_APIS_SERVER_URL";

  /* Statics */
  String VITE_REPO_URL_PREFIX = "VITE_REPO_URL_PREFIX";

  /* AngusRepo - Database */
  String REPO_DB_HOST = "REPO_DB_HOST";
  String REPO_DB_PORT = "REPO_DB_PORT";
  String REPO_DB_NAME = "REPO_DB_NAME";
  String REPO_DB_USER = "REPO_DB_USER";
  String REPO_DB_PASSWORD = "REPO_DB_PASSWORD";

  public ConfigurableRepoApplication() {
  }
  ...
  public void doConfigureApplication(ConfigurableEnvironment environment, Properties envs)
    ...
    if (appEdition.isPrivatization()) {
      ...
      if (isNotEmpty(installApps) && installApps.contains(REPO_SERVICE)) {
  ...
  private void rewriteEnvByBusiness() {
    ...
    envs.put(REPO_APIS_SERVER_URL, aiApisUrlPrefix);
    System.out.println("Set REPO_APIS_SERVER_URL to " + aiApisUrlPrefix);

    String testerWebSite = getTesterWebsite();
    envs.put(REPO_WEBSITE, testerWebSite);
    System.out.println("Set REPO_WEBSITE to " + testerWebSite);

    InstallType installType = getEnum(INSTALL_TYPE, InstallType.class, InstallType.SHARED);
    if (installType.isShared()) {
      envs.put(REPO_DB_HOST, getString(GM_DB_HOST));
      envs.put(REPO_DB_PORT, getString(GM_DB_PORT));
      envs.put(REPO_DB_NAME, getString(GM_DB_NAME));
      envs.put(REPO_DB_USER, getString(GM_DB_USER));
      envs.put(REPO_DB_PASSWORD, getString(GM_DB_PASSWORD));
    }
  }

  private void installApplication() throws Exception {
    // Configure all license info
    Pair<String, DCache> mainAppDCache = localDCaches.getOrDefault(REPO_SERVICE, new Pair<>());
  ...
  private void initDatabaseAndRedis() {
    ...
    boolean needInstall = appName.equalsIgnoreCase(REPO_SERVICE) && installApps.contains(appName);
    InstallType installType = getEnum(INSTALL_TYPE, InstallType.class, InstallType.SHARED);
    if (needInstall && installType.isStandalone()) {
      try {
        testerConnection = DatabaseChecker.checkConnection(databaseType,
            getString(REPO_DB_HOST, DEFAULT_HOST), getInt(REPO_DB_PORT, DEFAULT_MYSQL_PORT),
            getString(REPO_DB_NAME, DEFAULT_MYSQL_DB),
            getString(REPO_DB_USER, DEFAULT_MYSQL_USER),
            getString(REPO_DB_PASSWORD, DEFAULT_MYSQL_PASSWORD)
        );
        System.out.println("---> The AngusRepo database configuration is correct.");
      } catch (Exception e) {
        System.err.printf(
            "---> The AngusRepo database configuration is incorrect, cause: \n\t%s\n.",
            e.getMessage());
        throw new IllegalStateException(e.getMessage());
      }
    } else {
  ...
  private void updateApplicationWebsite() throws Exception {
    ...
    JDBCUtils.executeUpdate(gmConnection, "UPDATE app SET url=? WHERE code=?",
        Arrays.asList(getTesterWebsite(), REPO_SERVICE)
    );
    // @formatter:off
  }
  ...
  private void saveWebStaticsEnv() throws Exception {
    ...
    PropertiesRepo.ofPrivateStatics(staticPath)
        ...
        .save(VITE_REPO_URL_PREFIX, getTesterWebsite())
        .saveToDisk();
  }
  ...
  private void removeInstalledApplication() throws Exception {
    ...
    remoteApps.remove(REPO_SERVICE);
