...
import static cloud.xcan.angus.api.commonlink.client.enums.ClientSource.XCAN_SYS_TOKEN;
...
public class AuthenticationClientAssembler {
  ...
  public static AuthClientDetailVo toDetailVo(CustomOAuth2RegisteredClient client) {
    AuthClientDetailVo vo = new AuthClientDetailVo().setId(client.getId())
    ...
    // 设置审计信息
    vo.setTenantId(Long.valueOf(client.getTenantId()));
    vo.setCreatedBy(Long.valueOf(client.getCreatedBy()));
    vo.setCreatedDate(LocalDateTime.from(client.getCreatedDate()));
    vo.setModifiedBy(Long.valueOf(client.getLastModifiedBy()));
    vo.setModifiedDate(LocalDateTime.from(client.getLastModifiedDate()));
    return vo;
  }
