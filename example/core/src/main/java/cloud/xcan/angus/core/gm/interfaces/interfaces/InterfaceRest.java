...
public class InterfaceRest {
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "接口详情获取成功")
  })
  @ResponseStatus(HttpStatus.OK)
  @GetMapping("/{id}")
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "接口列表获取成功")
  })
  @ResponseStatus(HttpStatus.OK)
  @GetMapping
  public ApiLocaleResult<PageResult<InterfaceListVo>> list(
      @Valid @ParameterObject InterfaceFindDto dto) {
  ...
  public ApiLocaleResult<InterfaceCallStatsVo> getCallStats(
      @Parameter(description = "接口ID") @PathVariable Long id,
      @Valid @ParameterObject InterfaceCallStatsDto dto) {
