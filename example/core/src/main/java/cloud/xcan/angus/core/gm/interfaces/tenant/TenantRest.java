...
public class TenantRest {
  ...
  @ResponseStatus(HttpStatus.OK)
  @GetMapping
  public ApiLocaleResult<PageResult<TenantDetailVo>> list(
      @Valid @ParameterObject TenantFindDto dto) {
    return ApiLocaleResult.success(tenantFacade.list(dto));
  }

  @Operation(operationId = "getSameAccountTenants", summary = "查询同账号租户列表",
      description = "查询当前租户同账号下的所有租户信息（包含主账号及子账号）")
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "查询成功")
  })
  @ResponseStatus(HttpStatus.OK)
  @GetMapping("/same-account")
  public ApiLocaleResult<List<TenantDetailVo>> getSameAccountTenants() {
    return ApiLocaleResult.success(tenantFacade.getSameAccountTenants());
  }
