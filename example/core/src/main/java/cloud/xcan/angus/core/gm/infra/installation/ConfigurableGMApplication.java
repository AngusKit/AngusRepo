...
public class ConfigurableGMApplication implements ConfigurableApplication {
  ...
  private void installApplication() throws Exception {
    ...
    Long tenantId = getFinalTenantId(/*mainAppDCache.value*/null);

    addEnvForInstallSql(/*mainAppDCache.getValue()*/null);

    initDatabaseAndRedis();

    initDatabase();

    modifyApplicationConfiguration(tenantId, /*mainAppDCache*/null);
  ...
  private void modifyApplicationConfiguration(Long tenantId, /*Pair<String, DCache>*/
      Object mainAppDCache)
      throws Exception {
    // Configure the openapi2p client of store
    // saveStoreClient(dCache, gmDbConn);

    //saveLicense(/*localDCaches*/null); TODO

    // Configure the open info of AngusGM
    //saveApplicationOpen(tenantId, /*mainAppDCache.getValue()*/ null); TODO
...
//  }
  ...
  private void addEnvForInstallSql(Object/*DCache*/ mainDCache) {
