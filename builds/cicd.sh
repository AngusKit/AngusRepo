...
# ---------------------------------------------------------------------------
# Jenkins CI/CD Pipeline Script for AngusRepo Project.
...
clone_repository() {
  ...
  GIT_URL="https://github.com/xcancloud/AngusRepo.git"

  if [ -n "$gitCredential" ]; then
    GIT_URL="https://${gitCredential}@github.com/xcancloud/AngusRepo.git"
  fi
...
deploy_service() {
  ...
  ssh "$host" "cd ${REMOTE_APP_DIR} && sh shutdown-repo.sh" || {
  ...
  ssh "$host" "cd ${REMOTE_APP_DIR} && mkdir -p conf && mv classes/spring-logback.xml conf/repo-logback.xml" || {
  ...
  ssh "$host" "cd ${REMOTE_APP_DIR} && sh startup-repo.sh --debug" || {
...
deploy_private_edition() {
  ...
  dist_zip=$(ls dist/AngusRepo-*.zip)
...
deploy_web() {
  ...
  nginxFileName="${WEB_DIR}/nginx/nginx_${env##*.}_ai.conf"
